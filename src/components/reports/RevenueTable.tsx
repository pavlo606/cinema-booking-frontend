export function RevenueTable({ data }: any) {
  return (
    <div className="bg-surface p-4 rounded-xl mt-6">
      <p className="mb-4">Total revenue sold: {data.totalRevenue}</p>
      <table className="w-full text-left">
        ...
        <tbody className="text-text-primary">
          {data.screeningsRevenue.map((item: any) => (
            <tr key={item.screeningId} className="border-b border-gray-800">
              <td className="py-2">{item.filmTitle}</td>
